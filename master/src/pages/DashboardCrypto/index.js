...
const DashboardCrypto = () => {
  ...
  const handleSaveEdit = () => {
    setData(data.map(item => item.id === editingId ? newRow : item));
    setEditingId(null);
    setNewRow({
      id: '',
      name: '',
      ...
      <div className={`table-container ${showFilterPanel ? 'blurred' : ''}`}>
              ...
              {sortedColumns.map(column => (
                <td key={column.id} style={{ width: column.width }}>
                  {column.field === 'id' ? (
                    ...
                    <input
                      type={column.field === 'quantity' || column.field === 'price' ? 'number' : 'text'}
                      placeholder={column.title}
        ...
        <div className="pagination-controls">
          <div className="pagination-info">
            Showing {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, filteredAndSortedData.length)} of {filteredAndSortedData.length} items
          </div>
